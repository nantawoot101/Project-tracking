...
var View = FC.View = Model.extend({
	...
	getAllDayHtml: function() {
		...
		var text = this.opt('allDayText');
		if (language == 'th') {
			text = "ทั้งวัน"
		}
		return this.opt('allDayHtml') || htmlEscape(text);
	},
...
/*					add_y = 0;*/
				...
				if (lengthArr - 1 == i || parseInt(arrDate[i]) > 1000) {
					var yearBuddha = parseInt(arrDate[i]) + add_y;
...
var ListViewGrid = Grid.extend({
	...
	renderEmptyMessage: function () {
		...
		var text = this.view.opt('noEventsMessage');
		if (language == 'th') {
	...
	dayHeaderHtml: function(dayDate) {
		...
		var strDate = dayDate.format(altFormat);
		var arrDate = strDate.split(" ");
		var lengthArr = arrDate.length;
		var newstrDate = "";
		//deaw
			...
			'<td class="' + view.widgetHeaderClass + '" colspan="3">' +
				...
				(altFormat ?
					view.buildGotoAnchorHtml(
						...
						htmlEscape(newstrDate) // inner HTML
					) :
					'') +
			'</td>' +
		'</tr>';
