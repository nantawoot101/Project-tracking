...
(function ($) {
    ...
    function init(plot) {
        ...
        plot.ticksRedraw = function (xaxis, yaxis) {
            secondPass = false;
            var hasChange = false;
            $.each(plot.getAxes(), function (_, axis) {
                try {
                    ...
                    if ((xaxis != undefined && xaxis != null) && _ == 'xaxis') {
                        if (xaxis.hasOwnProperty('ticks')) {
                            opts.ticks = xaxis.ticks;
                            hasChange = true;
                        }
                    }
                    //yaxis
                    if ((yaxis != undefined && yaxis != null) && _ == 'yaxis') {
                        if (yaxis.hasOwnProperty('ticks')) {
                            opts.ticks = yaxis.ticks;
                            hasChange = true;
                        }
                    }
                } catch (e) { }
        ...
        plot.hooks.draw.push(function (plot, ctx) {
            ...
            if (!secondPass) {
                ...
                var elem, maxLabelWidth = 260, maxLabelHeight = 0, minX = -50, maxX = 50;
                ...
                //deaw check selection
                var new_ticks = [];
                var datalength = null;
                try {
                    datalength = xaxis.options.datalength;
                } catch (e) { }
                try {
                    for (var i = 0; i < ticks.length; i++) {
                        var ticks_index = ticks[i].v;
                        if ((ticks_index >= xaxis.min && ticks_index <= xaxis.max) || (datalength - 1 == xaxis.max)) {
                            new_ticks.push(ticks[i]);
                        }
                    }
                } catch (e) { }
                ticks = new_ticks;
